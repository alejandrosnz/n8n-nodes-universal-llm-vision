...
/**
 * Builds the multimodal message content with text and audio
 */
export function buildAudioMessageContent(
  prompt: string,
  audioContent: {
    data: string;
    mimeType: string;
...
/**
 * @deprecated Use buildImageMessageContent instead
 */
export function buildMessageContent(
  ...
  return buildImageMessageContent(prompt, imageContent, options);
}

/**
 * Builds the complete messages array including optional system prompt
 */
export function buildMessagesArray(messageContent: any[], systemPrompt?: string): any[] {
  const messages: any[] = [];
